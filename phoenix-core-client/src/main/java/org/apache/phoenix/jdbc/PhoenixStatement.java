...
public class PhoenixStatement implements PhoenixMonitoredStatement, SQLCloseable {
    ...
    private PhoenixResultSet executeQuery(final CompilableStatement stmt,
        ...
        // 日志增强开始
        final long logStartTime = System.currentTimeMillis();
        String sqlText = (stmt != null) ? stmt.toString() : "<null>";
        String user = System.getProperty("user.name", "unknown");
        String thread = Thread.currentThread().getName();
        List<Object> params = null;
        if (this instanceof PhoenixPreparedStatement) {
            try {
                params = ((PhoenixPreparedStatement)this).getParameters();
            } catch (Exception e) {
                params = null;
            }
        }
        LOGGER.info("[SQL-EXEC-START] user={}, thread={}, sql=\"{}\", params={}, startTime={}", user, thread, sqlText, params, logStartTime);
        LOGGER.info("[SQL-EXEC-START-JSON] {}", String.format("{\"user\":\"%s\",\"thread\":\"%s\",\"sql\":\"%s\",\"params\":%s,\"startTime\":%d}", user, thread, sqlText.replace("\"","\\\""), params, logStartTime));
        try {
            PhoenixResultSet result = CallRunner
                    .run(new CallRunner.CallableThrowable<PhoenixResultSet, SQLException>() {
                        @Override public PhoenixResultSet call() throws SQLException {
                            ...
                            long duration = logEndTime - logStartTime;
                            int rowCount = (rs != null && rs.getStatement() != null) ? rs.getStatement().getUpdateCount() : -1;
                            LOGGER.info("[SQL-EXEC-END] user={}, thread={}, sql=\"{}\", params={}, endTime={}, duration={}ms, rowCount={}", user, thread, sqlText, params, logEndTime, duration, rowCount);
                            LOGGER.info("[SQL-EXEC-END-JSON] {}", String.format("{\"user\":\"%s\",\"thread\":\"%s\",\"sql\":\"%s\",\"params\":%s,\"endTime\":%d,\"duration\":%d,\"rowCount\":%d}", user, thread, sqlText.replace("\"","\\\""), params, logEndTime, duration, rowCount));
                            return rs;
                        }
                    }, PhoenixContextExecutor.inContext());
            long logEndTime = System.currentTimeMillis();
            long duration = logEndTime - logStartTime;
            int rowCount = (result != null && result.getStatement() != null) ? result.getStatement().getUpdateCount() : -1;
            LOGGER.info("[SQL-EXEC-END] user={}, thread={}, sql=\"{}\", params={}, endTime={}, duration={}ms, rowCount={}", user, thread, sqlText, params, logEndTime, duration, rowCount);
            LOGGER.info("[SQL-EXEC-END-JSON] {}", String.format("{\"user\":\"%s\",\"thread\":\"%s\",\"sql\":\"%s\",\"params\":%s,\"endTime\":%d,\"duration\":%d,\"rowCount\":%d}", user, thread, sqlText.replace("\"","\\\""), params, logEndTime, duration, rowCount));
            return result;
        } catch (Exception ex) {
            long logEndTime = System.currentTimeMillis();
            long duration = logEndTime - logStartTime;
            LOGGER.error("[SQL-EXEC-ERROR] user={}, thread={}, sql=\"{}\", params={}, endTime={}, duration={}ms, error={}", user, thread, sqlText, params, logEndTime, duration, ex.getMessage(), ex);
            LOGGER.error("[SQL-EXEC-ERROR-JSON] {}", String.format("{\"user\":\"%s\",\"thread\":\"%s\",\"sql\":\"%s\",\"params\":%s,\"endTime\":%d,\"duration\":%d,\"error\":\"%s\"}", user, thread, sqlText.replace("\"","\\\""), params, logEndTime, duration, ex.getMessage()));
    ...
    private Pair<Integer, ResultSet> executeMutation(final CompilableStatement stmt,
        ...
        LOGGER.info("[SQL-EXEC-START-JSON] {}", String.format("{\"user\":\"%s\",\"thread\":\"%s\",\"sql\":\"%s\",\"params\":%s,\"startTime\":%d}", user, thread, sqlText.replace("\"","\\\""), params, logStartTime));
        try {
            Pair<Integer, ResultSet> result = CallRunner
                   .run(
                        ...
                        @Override
                            public Pair<Integer, ResultSet> call() throws SQLException {
                                ...
                                long logEndTime = System.currentTimeMillis();
                                long duration = logEndTime - logStartTime;
                                int updateCount = (success) ? lastUpdateCount : -1;
                                LOGGER.info("[SQL-EXEC-END] user={}, thread={}, sql=\"{}\", params={}, endTime={}, duration={}ms, updateCount={}", user, thread, sqlText, params, logEndTime, duration, updateCount);
                                LOGGER.info("[SQL-EXEC-END-JSON] {}", String.format("{\"user\":\"%s\",\"thread\":\"%s\",\"sql\":\"%s\",\"params\":%s,\"endTime\":%d,\"duration\":%d,\"updateCount\":%d}", user, thread, sqlText.replace("\"","\\\""), params, logEndTime, duration, updateCount));
                                return new Pair<>(updateCount, getLastResultSet());
                            }
                            //Force update cache and retry if meta not found error occurs
                            catch (MetaDataEntityNotFoundException e) {
                                if (doRetryOnMetaNotFoundError && e.getTableName() != null) {
            ...
            long logEndTime = System.currentTimeMillis();
            long duration = logEndTime - logStartTime;
            int updateCount = (result != null) ? result.getFirst() : -1;
            LOGGER.info("[SQL-EXEC-END] user={}, thread={}, sql=\"{}\", params={}, endTime={}, duration={}ms, updateCount={}", user, thread, sqlText, params, logEndTime, duration, updateCount);
            LOGGER.info("[SQL-EXEC-END-JSON] {}", String.format("{\"user\":\"%s\",\"thread\":\"%s\",\"sql\":\"%s\",\"params\":%s,\"endTime\":%d,\"duration\":%d,\"updateCount\":%d}", user, thread, sqlText.replace("\"","\\\""), params, logEndTime, duration, updateCount));
            return result;
        } catch (Exception ex) {
